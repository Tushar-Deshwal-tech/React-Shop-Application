import Mens_Classic_TShirt1 from './MensProductsImages/Mens_Classic_T-Shirt1.jpg';
import Mens_Classic_TShirt2 from './MensProductsImages/Mens_Classic_T-Shirt2.jpg';

import Mens_Slim_Fit_Jeans1 from './MensProductsImages/Mens_Slim_Fit_Jeans1.jpg';
import Mens_Slim_Fit_Jeans2 from './MensProductsImages/Mens_Slim_Fit_Jeans2.jpg';

import Mens_Leather_Jacket1 from './MensProductsImages/Mens_Leather_Jacket1.jpg';
import Mens_Leather_Jacket2 from './MensProductsImages/Mens_Leather_Jacket2.jpg';

import Mens_Hoodie1 from './MensProductsImages/Mens_Hoodie1.jpg';
import Mens_Hoodie2 from './MensProductsImages/Mens_Hoodie2.jpg';

import Mens_Chinos1 from './MensProductsImages/Mens_Chinos1.jpg';
import Mens_Chinos2 from './MensProductsImages/Mens_Chinos2.jpg';

import Mens_Polo_Shirt1 from './MensProductsImages/Mens_Polo_Shirt1.jpg';
import Mens_Polo_Shirt2 from './MensProductsImages/Mens_Polo_Shirt2.jpg';

import Mens_Winter_Coat1 from './MensProductsImages/Mens_Winter_Coat1.jpg';
import Mens_Winter_Coat2 from './MensProductsImages/Mens_Winter_Coat2.jpg';

import Mens_Swim_Trunks1 from './MensProductsImages/Mens_Swim_Trunks1.jpg';
import Mens_Swim_Trunks2 from './MensProductsImages/Mens_Swim_Trunks2.jpg';

import Mens_Cargo_Pants1 from './MensProductsImages/Mens_Cargo_Pants1.jpg';
import Mens_Cargo_Pants2 from './MensProductsImages/Mens_Cargo_Pants2.jpg';

import Mens_Dress_Shirt1 from './MensProductsImages/Mens_Dress_Shirt1.jpg';
import Mens_Dress_Shirt2 from './MensProductsImages/Mens_Dress_Shirt2.jpg';

import Mens_Sweatpants1 from './MensProductsImages/Mens_Sweatpants1.jpg';
import Mens_Sweatpants2 from './MensProductsImages/Mens_Sweatpants2.jpg';

import Mens_Blazer1 from './MensProductsImages/Mens_Blazer1.jpg';
import Mens_Blazer2 from './MensProductsImages/Mens_Blazer2.jpg';

import Mens_V_Neck_Sweater1 from './MensProductsImages/Mens_V_Neck_Sweater1.webp';
import Mens_V_Neck_Sweater2 from './MensProductsImages/Mens_V_Neck_Sweater2.webp';

import Mens_Shorts1 from './MensProductsImages/Mens_Shorts1.jpg';
import Mens_Shorts2 from './MensProductsImages/Mens_Shorts2.jpg';

import Mens_Trench_Coat1 from './MensProductsImages/Mens_Trench_Coat1.jpg';
import Mens_Trench_Coat2 from './MensProductsImages/Mens_Trench_Coat2.jpg';

import Mens_Tank_Top1 from './MensProductsImages/Mens_Tank_Top1.jpg';
import Mens_Tank_Top2 from './MensProductsImages/Mens_Tank_Top2.jpg';

import Mens_Wool_Sweater1 from './MensProductsImages/Mens_Wool_Sweater1.jpg';
import Mens_Wool_Sweater2 from './MensProductsImages/Mens_Wool_Sweater2.jpg';

import Mens_Athletic_Shorts1 from './MensProductsImages/Mens_Athletic_Shorts1.jpg';
import Mens_Athletic_Shorts2 from './MensProductsImages/Mens_Athletic_Shorts2.jpg';

import Mens_Cardigan1 from './MensProductsImages/Mens_Cardigan1.jpg';
import Mens_Cardigan2 from './MensProductsImages/Mens_Cardigan2.jpg';

import Mens_Casual_Shirt1 from './MensProductsImages/Mens_Casual_Shirt1.jpg';
import Mens_Casual_Shirt2 from './MensProductsImages/Mens_Casual_Shirt2.jpg';

import Mens_Sports_Jacket1 from './MensProductsImages/Mens_Sports_Jacket1.jpg';
import Mens_Sports_Jacket2 from './MensProductsImages/Mens_Sports_Jacket2.jpg'; 

const Men_Products_Data = [
  {
    id: 1,
    productName: "Men's Classic T-Shirt",
    imageLink1: Mens_Classic_TShirt1,
    imageLink2: Mens_Classic_TShirt2,
    description: "A comfortable and stylish classic t-shirt for everyday wear.",
    price: 19.99,
    rating: 4.5,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 2,
    productName: "Men's Slim Fit Jeans",
    imageLink1: Mens_Slim_Fit_Jeans1,
    imageLink2: Mens_Slim_Fit_Jeans2,
    description: "Slim fit jeans made with high-quality denim fabric.",
    price: 49.99,
    rating: 4.7,
    sizes: ["30", "32", "34", "36", "38"]
  },
  {
    id: 3,
    productName: "Men's Leather Jacket",
    imageLink1: Mens_Leather_Jacket1,
    imageLink2: Mens_Leather_Jacket2,
    description: "A premium leather jacket for a rugged and stylish look.",
    price: 129.99,
    rating: 4.8,
    sizes: ["M", "L", "XL", "XXL"]
  },
  {
    id: 4,
    productName: "Men's Hoodie",
    imageLink1: Mens_Hoodie1,
    imageLink2: Mens_Hoodie2,
    description: "A cozy and warm hoodie for a relaxed, casual look.",
    price: 29.99,
    rating: 4.4,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 5,
    productName: "Men's Chinos",
    imageLink1: Mens_Chinos1,
    imageLink2: Mens_Chinos2,
    description: "Versatile and comfortable chinos for both casual and semi-formal occasions.",
    price: 34.99,
    rating: 4.6,
    sizes: ["30", "32", "34", "36", "38"]
  },
  {
    id: 6,
    productName: "Men's Polo Shirt",
    imageLink1: Mens_Polo_Shirt1,
    imageLink2: Mens_Polo_Shirt2,
    description: "A classic polo shirt with a modern fit.",
    price: 24.99,
    rating: 4.2,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 7,
    productName: "Men's Winter Coat",
    imageLink1: Mens_Winter_Coat1,
    imageLink2: Mens_Winter_Coat2,
    description: "A heavy-duty winter coat to keep you warm in the coldest weather.",
    price: 89.99,
    rating: 4.7,
    sizes: ["M", "L", "XL", "XXL"]
  },
  {
    id: 8,
    productName: "Men's Swim Trunks",
    imageLink1: Mens_Swim_Trunks1,
    imageLink2: Mens_Swim_Trunks2,
    description: "Comfortable and stylish swim trunks for the beach or pool.",
    price: 19.99,
    rating: 4.4,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 9,
    productName: "Men's Cargo Pants",
    imageLink1: Mens_Cargo_Pants1,
    imageLink2: Mens_Cargo_Pants2,
    description: "Durable cargo pants with multiple pockets.",
    price: 44.99,
    rating: 4.6,
    sizes: ["30", "32", "34", "36", "38"]
  },
  {
    id: 10,
    productName: "Men's Dress Shirt",
    imageLink1: Mens_Dress_Shirt1,
    imageLink2: Mens_Dress_Shirt2,
    description: "A sleek dress shirt perfect for formal and business occasions.",
    price: 39.99,
    rating: 4.5,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 11,
    productName: "Men's Sweatpants",
    imageLink1: Mens_Sweatpants1,
    imageLink2: Mens_Sweatpants2,
    description: "Comfortable and casual sweatpants for lounging or exercise.",
    price: 29.99,
    rating: 4.4,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 12,
    productName: "Men's Blazer",
    imageLink1: Mens_Blazer1,
    imageLink2: Mens_Blazer2,
    description: "A stylish blazer for semi-formal and formal events.",
    price: 99.99,
    rating: 4.7,
    sizes: ["38", "40", "42", "44", "46"]
  },
  {
    id: 13,
    productName: "Men's V-Neck Sweater",
    imageLink1: Mens_V_Neck_Sweater1,
    imageLink2: Mens_V_Neck_Sweater2,
    description: "A classic V-neck sweater for a smart casual look.",
    price: 49.99,
    rating: 4.6,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 14,
    productName: "Men's Shorts",
    imageLink1: Mens_Shorts1,
    imageLink2: Mens_Shorts2,
    description: "Comfortable and versatile shorts for casual wear.",
    price: 24.99,
    rating: 4.3,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 15,
    productName: "Men's Trench Coat",
    imageLink1: Mens_Trench_Coat1,
    imageLink2: Mens_Trench_Coat2,
    description: "A stylish trench coat for a sophisticated look.",
    price: 129.99,
    rating: 4.8,
    sizes: ["M", "L", "XL", "XXL"]
  },
  {
    id: 16,
    productName: "Men's Tank Top",
    imageLink1: Mens_Tank_Top1,
    imageLink2: Mens_Tank_Top2,
    description: "A comfortable tank top for workouts or casual wear.",
    price: 14.99,
    rating: 4.2,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 17,
    productName: "Men's Wool Sweater",
    imageLink1: Mens_Wool_Sweater1,
    imageLink2: Mens_Wool_Sweater2,
    description: "A warm wool sweater for cold weather.",
    price: 69.99,
    rating: 4.7,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 18,
    productName: "Men's Athletic Shorts",
    imageLink1: Mens_Athletic_Shorts1,
    imageLink2: Mens_Athletic_Shorts2,
    description: "Lightweight and breathable athletic shorts.",
    price: 19.99,
    rating: 4.5,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 19,
    productName: "Men's Cardigan",
    imageLink1: Mens_Cardigan1,
    imageLink2: Mens_Cardigan2,
    description: "A stylish cardigan for layering and comfort.",
    price: 39.99,
    rating: 4.6,
    sizes: ["S", "M", "L", "XL", "XXL"]
  },
  {
    id: 20,
    productName: "Men's Casual Shirt",
    imageLink1: Mens_Casual_Shirt1,
    imageLink2: Mens_Casual_Shirt2,
    description: "A versatile and comfortable casual shirt for everyday wear.",
    price: 29.99,
    rating: 4.3,
    sizes: ["S", "M", "L", "XL"]
  },
  {
    id: 21,
    productName: "Men's Sports Jacket",
    imageLink1: Mens_Sports_Jacket1,
    imageLink2: Mens_Sports_Jacket2,
    description: "A stylish sports jacket suitable for both casual and semi-formal occasions.",
    price: 79.99,
    rating: 4.6,
    sizes: ["M", "L", "XL", "XXL"]
  }
];

export default Men_Products_Data;
